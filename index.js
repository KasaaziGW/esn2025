...
app.post("/login", (req, res) => {
  ...
  Citizen.findOne({ email }).then((citizen) => {
    if (!citizen) {
      req.flash("error", "Citizen does not exist!");
      return res.redirect("/");
    }
    bcrypt.compare(password, citizen.password, async (err, result) => {
      if (result) {
        await Citizen.updateOne({ email }, { $set: { online: true } });
        req.session.user = { fullname: citizen.fullname, email: citizen.email };
