 ...
 */
export const joinCommunity = errorHandler.catchAsync(async (req, res, next) => {
  ...
  if (!user.region || !user.district) {
    throw new errorHandler.ValidationError('Please complete your profile by setting your region and district before joining a community');
  }
  ...
  if (['citizen', 'coordinator'].includes(user.role) && user.community) {
    throw new errorHandler.ValidationError('You must leave your current community before joining another');
  }
  ...
  if (user.region._id.toString() !== community.region._id.toString() || 
    ...
    throw new errorHandler.ValidationError('You can only join communities in your current region and district. Please update your profile if you want to join communities in a different location.');
  }
  ...
  response.sendOK(res, 'Joined community successfully', { 
 ...
 */
export const leaveCommunity = errorHandler.catchAsync(async (req, res, next) => {
  const { communityId } = req.params;
  const userId = req.user._id;
  ...
  if (!membership) throw new errorHandler.ValidationError('You are not a member of this community');
  ...
  response.sendOK(res, 'Left community successfully', { membersCount: community.membersCount });
});
 ...
 */
export const listMembers = errorHandler.catchAsync(async (req, res, next) => {
  const { communityId } = req.params;
  const user = req.user;
  ...
  response.sendOK(res, 'Community members retrieved successfully', { total: members.length, members });
});
 ...
 */
export const assignCoordinator = errorHandler.catchAsync(async (req, res, next) => {
  ...
  const membership = await CommunityMember.findOne({ community: communityId, user: userId });
  if (!membership) throw new errorHandler.NotFoundError('User is not a member of this community');

  membership.role = 'coordinator';
  await membership.save();

  response.sendOK(res, 'Coordinator role assigned successfully', { member: membership });
});
